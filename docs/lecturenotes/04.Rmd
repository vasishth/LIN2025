...
Next, we revisit the effect of (centered) trial id on button-pressing times. Recall from the chapter 3 lecture that 
we used the log-normal likelihood.
...
In the code below, `sample_prior="only"` ensures that we generate data from the priors.
`df_spacebar_ref` is a dummy data set with no actual data in it; it's just there because `brm` always needs 
the data to be specified.
...
We could plot the prior predictive distribution of a particular statistic of interest, 
for example: the prior predictive distribution of the median difference in button-tapping times between adjacent trials.
...
```{r, echo=FALSE, results='asis'}
cat('
<img src="figures/fig1oberauer2019modified.png" alt="BDA cover" style="height:600px;">
')
```
